...
function $rpcProvider() {
    ...
    return { 
        ...
        connect: function(url){
            ...
            rpc.socket = new WebSocket(url);

            rpc.socket.onopen = function(){
                // HeartBeat
                setInterval(function(){ 
                    rpc.socket.send('ping');
                }, 20000);
            }

            rpc.socket.onclose = function(event){
                if (event.wasClean) {
                    console.log('Clean close', event);
                } else {
                    console.log('Connection refused', event);
                }
            }

            rpc.socket.onmessage = function(event){
                ...
                if (listeners[uuid]){
                    if (status == 'SUCCESS'){
                        listeners[uuid].resolve(data);
                    } else {
                        listeners[uuid].reject(data);
                    }
                    delete listeners[uuid];
                }
            } 

            rpc.socket.onerror = function(error){
                console.error(error);
            }
        },
        ...
        $get: function($q){
            function send(func, data){
                ...
                listeners[data.uuid] = $q.defer();
                rpc.socket.send(JSON.stringify(data));
                return listeners[data.uuid].promise;
            };
            return {
                send: send,
            };
...
ngRpc.provider('$rpc', $rpcProvider);
;/**
